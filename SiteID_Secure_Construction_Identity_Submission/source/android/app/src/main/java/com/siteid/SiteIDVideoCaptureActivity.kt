...
class SiteIDVideoCaptureActivity : Activity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    framesDir.listFiles()
      ?.filter { it.name.startsWith("siteid_preview_frame_") }
      ?.forEach { it.delete() }
  ...
  private fun capturePreviewFrame(index: Int) {
    try {
      ...
      val frameFile = File(
        framesDir,
        "siteid_preview_frame_${index}_${System.currentTimeMillis()}.jpg",
      )
  ...
  private fun startBackgroundThread() {
    backgroundThread = HandlerThread("SiteIDVideoCamera").also { thread ->
      thread.start()
  ...
  companion object {
    private const val TAG = "SiteIDVideo"
