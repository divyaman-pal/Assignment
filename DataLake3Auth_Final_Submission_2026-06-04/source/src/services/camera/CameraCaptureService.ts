...
async function requestCameraPermission(): Promise<void> {
  ...
  const result = await PermissionsAndroid.request(
    PermissionsAndroid.PERMISSIONS.CAMERA,
    {
      ...
      message: 'DataLake3Auth needs camera access to capture face videos.',
...
export async function captureFaceVideo(
  ...
  if (!DataLakeCamera?.captureVideo) {
    throw new Error('Native video capture module is not available.');
  }

  await requestCameraPermission();
  const result = await DataLakeCamera.captureVideo(durationMs, frameCount);
...
export async function preprocessFaceImage(
  ...
  if (!DataLakeCamera?.preprocessFace) {
    throw new Error('Native face preprocessing module is not available.');
  }

  const values: number[] = await DataLakeCamera.preprocessFace(
    imageUri,
    bounds,
    targetSize,
  );
